...
date: 2017-12-16 14:46:39
coverImage: https://cdn-images-1.medium.com/max/2000/1*ZpwZLFDNYZodJDerr7-37A.png
sourceUrl: https://medium.com/reloading/javascript-start-up-performance-69200f43b201
tags: [javascript]
slug: /javascript-start-up-performance
---

# JavaScript Start-up Performance

As web developers, we know how easy it is to end up with web page bloat. But **loading** a webpage is much more than shipping bytes down the wire. Once the browser has downloaded our page’s scripts it then has to parse, interpret & run them. In this post, we’ll dive into this phase for JavaScript, _why_ it might be slowing down your app’s start-up & _how_ you can fix it.
...
![](https://cdn-images-1.medium.com/max/2000/1*GuWInZljjvtDpdeT6O0emA.png)A simplified view of how V8 works. This is our idealized pipeline that we’re working towards.
...
![](https://cdn-images-1.medium.com/max/2000/0*M94-AavlZjGoudZG.)Parse & Compile times for a popular website using V8’s Runtime Call Stats in Chrome Canary. Notice how a slow Parse/Compile on desktop can take far longer on average mobile phones.
...
![](https://cdn-images-1.medium.com/max/2000/1*XjHkzz0B7KlcDbLFD1JS8Q.png)The pink area (JavaScript) represents time spent in V8 and Blink’s C++, while the orange and yellow represent parse and compile.
...
<Gist id="https://medium.com/media/bfe5fc0258489a112c44c78f4de550d9?postId=69200f43b201" />

![](https://cdn-images-1.medium.com/max/1200/1*nkJwMuE5PpgF_pE0e6RM6g.jpeg)Sam Saccone calls out the cost of JS parse in ‘[Planning for Performance](https://www.youtube.com/watch?v=RWLzUnESylc)’
...
I spent two months [digging into](https://github.com/GoogleChrome/discovery/issues/1) the performance of a large set of production sites (6000+) built with different libraries and frameworks — like React, Angular, Ember and Vue. Most of the tests were recently redone on WebPageTest so you can easily redo them yourself or dig into the numbers if you wish. Here are some insights.

**Apps became interactive in 8 seconds on desktop (using cable) and 16 seconds on mobile (Moto G4 over 3G)**

![](https://cdn-images-1.medium.com/max/2000/1*WC4zanI0DKAoSiJVU3VUeA.png)

**What contributed to this? Most apps spent an average of 4 seconds in start-up (Parse/Compile/Exec)..on desktop.**

![](https://cdn-images-1.medium.com/max/1600/1*NacL9cZJ1osZowPS6hbCsQ.jpeg)

On mobile, parse times were up to 36% higher than they were on desktop.

![](https://cdn-images-1.medium.com/max/2000/1*uTRfB5pne06h8lp5jGtiIQ.jpeg)
...
![](https://cdn-images-1.medium.com/max/1600/1*GvwfE2GjKQyLBKPmmfRwuA.png)[HTTPArchive stat](http://httparchive.org/trends.php?s=All&minlabel=Nov+15+2015&maxlabel=Nov+15+2016#bytesJS&reqJS): the average page ships down 420KB of JavaScript
...
![](https://cdn-images-1.medium.com/max/1600/0*P-_pLIITtYJRikRN.)

**WebPageTest**

![](https://cdn-images-1.medium.com/max/1200/1*y6x_vr7aOxK4jHG9blgseg.png)
...
![](https://cdn-images-1.medium.com/max/1600/1*tV48evC-XzYkoHonyKGkOw.png)
...
<Gist id="https://medium.com/media/3a4c46356a6de348271a00bff1d4d2d4?postId=69200f43b201" />

The third `<script>` here isn’t important, but it’s the first `<script>` being separate from the second (_performance.mark()_ starting before the `<script>` has been reached) that is.

_This approach can be affected on subsequent reloads by V8’s preparser. This could be worked around by appending a random string to the end of the script, something Nolan does in his optimize-js benchmarks._

I use a similar approach for measuring the impact of JavaScript Parse times using Google Analytics:

![](https://cdn-images-1.medium.com/max/1600/1*ziA8f9KhB1gOt-Mq07cRFw.jpeg)A custom Google Analytics dimension for ‘parse’ allows me to measure JavaScript parse times from real users and devices hitting my pages in the wild.
...
![](https://cdn-images-1.medium.com/max/1600/1*FFzrH2QUiQZFX2rlF5e2-g.jpeg)
...
![](https://cdn-images-1.medium.com/max/1600/1*sr4eb-cx3lq7hVrJGfDNaw.png)Nolan Lawson’s ‘[Solving the Web Performance Crisis](https://channel9.msdn.com/Blogs/msedgedev/nolanlaw-web-perf-crisis)’

### **What are _browsers_ doing to improve Parse & Compile times today?**
...
<Gist id="https://medium.com/media/f34895d2d5798e73d1987965288e5a55?postId=69200f43b201" />

We also estimate a 25% improve on V8 parse times for many pages looking at our Octane-Codeload numbers:

![](https://cdn-images-1.medium.com/max/1600/1*cE8uvuvb0-iZslygh2NCTQ.jpeg)
...
![](https://cdn-images-1.medium.com/max/1200/1*xChjWSbT1rCqgLMacOMotQ.png)From [using V8’s code caching](https://www.nativescript.org/blog/using-v8-code-caching-to-minimize-app-load-time-on-android)
...
We can play around with these in _chrome://flags/#v8-cache-strategies-for-cache-storage_ to look at the difference. We can also run Chrome with — js-flags=profile-deserialization to see if items are being loaded from the code cache (these are presented as deserialization events in the log).
...
![](https://cdn-images-1.medium.com/max/2000/1*ooXJ0NES-gXEzteaGPL2nQ.png)

Since the feature was first introduced, V8 have switched over to allowing **all scripts**, _even_ parser blocking `<script src=””>` to be parsed on a background thread so everyone should be seeing some wins here. The only caveat is that there’s only one streaming background thread and so it makes sense to put our large/critical scripts in here first. _It’s important to measure for any potential wins here._
...
![](https://cdn-images-1.medium.com/max/1600/1*FAvUG7DrVJUXCK3oweMSLQ.png)
...
`(function (global, **module**) { … })(this, function **module**() { _my functions_ })`
...
V8 are already working on collecting info about inner functions during the initial compile, so any future compilations can _ignore_ their inner functions. For **module**-style functions, this should result in a large perf improvement.
...
![](https://cdn-images-1.medium.com/max/1200/1*LMRg_jHJeP53vdy8aiTEJQ.png)
...
_V8 are also working on reducing the cost for cases where we guess wrong, and that should also reduce the need for the parens hack_
...
_With thanks to V8 (Toon Verwaest, Camillo Bruni, Benedikt Meurer, Marja Hölttä, Seth Thompson), Nolan Lawson (MS Edge), Malte Ubl (AMP), Tim Kadlec (Synk), Gray Norton (Chrome DX), Paul Lewis, Matt Gaunt and Rob Wormald (Angular) and for their reviews of this article._

**Update:** Thanks to some awesome members of the community, this article is now available in [Chinese](https://mp.weixin.qq.com/s?__biz=MzIwNjQwMzUwMQ==&mid=2247484987&idx=1&sn=7f20da20bc6baed62ca8ff115209942b&chksm=972364f9a054edefccebc89bb4b39150328f84fc6a3da53dafa9563df7375fef00b3a1a4c483&mpshare=1) and [Russian](https://habrahabr.ru/company/mailru/blog/321748/) too.
